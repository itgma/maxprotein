...
std::unique_ptr<FoodVector> filter_food_vector(const FoodVector& source,
  ...
  // TODO: implement this function, then delete this comment
	std::unique_ptr<FoodVector> new_vector(new FoodVector);
	
for (auto& food : source) {
	if (food->kcal() != 0) {
		if (food->kcal() >= min_kcal || food->kcal() <= max_kcal) {
			if (new_vector->size() < total_size) {
				new_vector->push_back(food);
			}
		}
...
return new_vector;
	...
	std::unique_ptr<FoodVector> greedy_max_protein(const FoodVector& foods,
		...
		for (int i = 0; i < foods.size(); i++) {
			copy.push_back(foods[i]);
		}
		while (result_kcal < total_kcal) {

			int bp = copy[0]->protein_g();
			int bc = copy[0]->kcal();
			int bi = 0;
			for (int i = 1; i < copy.size(); i++) {
				if (copy[i]->protein_g() > bp) {

					if (result_kcal + copy[i]->kcal() <= total_kcal) {
						bc = copy[i]->kcal();
						bi = i;
						bp = copy[i]->protein_g();
					}
			...
			if (result_kcal + bc <= total_kcal) {
				result->push_back(copy[bi]);
				result_kcal += bc;
				std::swap(copy[bi], copy[copy.size() - 1]);
				copy.pop_back();
			}
			else
				break;
	...
	std::unique_ptr<FoodVector> exhaustive_max_protein(const FoodVector& foods,
		...
		for (i = 0; i < num; i++) {
			int result_kcal = 0;
			int result_protein_g = 0;
			candidate->clear();
			for (j = 0; j < n; j++) {
				if (((i >> j) & 1) == 1) {
					candidate->push_back(foods[j]);
					result_kcal += foods[j]->kcal();
					result_protein_g += foods[j]->protein_g();
				}
			}
			if (result_kcal <= total_kcal) {
				if (best->empty() || result_protein_g > bp) {

					best->clear();
					for (int a = 0; a < candidate->size(); a++) {
						best->push_back((*candidate)[a]);
					}
